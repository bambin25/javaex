...
public class StreamDemo2 {
  public static void main(String[] args) {
    ...
    // IntStream, DoubleStream, LongStream - 숫자 스트림 : average(), sum() 통계메서드 제공
    int[] iarr = {1,2,3,4,5}; // 배열, 공간적 집합체
    IntStream iStream = Arrays.stream(iarr);// stream 이라는 시간적 집합체
    System.out.println(iStream.sum() + ":");
    iStream = Arrays.stream(iarr);
    System.out.println(iStream.average());

    double[] darr = {1.0,2.0,3.0,4.0};
    DoubleStream dStream = Arrays.stream(darr);// LongStream 이라는 시간적 집합체

    long[] larr = {1L, 2L, 3L, 4L};
    LongStream lStream = Arrays.stream(larr);
    ...
    Stream<Integer> stream1 = ilist.stream();// stream 이라는 시간적 집합체
